...
if __name__ == '__main__':
    ...
    output = "\n".join(
        (
            f"Studentów jest {len(df.index)}",
            f"Średnia liczba studentów per uniwersytet {grouped_uniwersytet.count().mean()['id_studenta']}",
            f"Najczęściej imie "
            f"{df[['imie', 'id_studenta']].groupby('imie').count().sort_values('id_studenta').index[-1]}",
            f"Najrzadziej nazwisko "
            f"{df[['nazwisko', 'id_studenta']].groupby('nazwisko').count().sort_values('id_studenta').index[0]}",
            f"Sredni wiek {df['wiek'].mean()}",
            f"Srednia ocen {df['srednia_ocen'].mean()}",
            f"Srednia ocen per uniwersytet:\n{grouped_uniwersytet['srednia_ocen'].mean().to_string()}",
        )
    )
